...
func (cw *ChainWriter) WriteBlock(serializedBlock []byte) *FileInfo {
	// 1. need to know the length of the block
	
	// 2. if we don't have enough space for this block in the current file,
	...
	// 3. create path to correct file, following format
	// "DataDirectory/BlockFileName_CurrentBlockFileNumber.FileExtension"
	// Ex: "data/block_0.txt"
	
	// 4. write serialized block to disk
	
	// 5. create a file info object with the starting and ending offsets of the serialized block
...
func (cw *ChainWriter) WriteUndoBlock(serializedUndoBlock []byte) *FileInfo {
	...
	// 2. if we don't have enough space for this undo block in the current undo file,
	...
	// 4. write serialized undo block to disk
	
	// 5. create a file info object with the starting and ending undo offsets of the serialized
	// undo block
