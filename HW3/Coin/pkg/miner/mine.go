...
func (m *Miner) Mine() *block.Block {
	// 1. get a new context for the goroutine that we're about to spawn
	// should only mind if our transaction pool has enough priority
	
	// 2. We are currently mining
	
	// 3. create a new mining pool (get the highest priority transactions)
	
	// 4. have to insert the coinbase transaction at the top of the transactions list
	
	// 5. this is the block that we're going to mine!
	
	// 6. if results is true, we found a winning nonce! Otherwise, we failed (which won't ever actually happen
	// for us)
	// Change this to something else

	// 7. done mining

	// 8. send the block to the node to handle
...
func (m *Miner) GenerateCoinbaseTransaction(txs []*block.Transaction) *block.Transaction {
	// 1. first collect the fees for all the transactions

	// 2. find out what the minting reward is

	// 3. get our public key, so that we can send the txo to ourselves

	// 4. Output with fee reward and minting reward to ourselves
	
	// 5. the actual transaction. Note: no inputs since Coinbase!
