...
type Node struct {
	*pro.UnimplementedCoinServer
	...
	SeenTransactions map[string]uint32
...
func New(conf *Config) *Node {
	...
	n.SeenTransactions = make(map[string]uint32)
	n.SeenBlocks = make(map[string]uint32)
...
func (n *Node) BroadcastTransaction(tx *block.Transaction) {
  // 1.update SeenTransactions and check whether the miner exits

	// 2. Students can either wrap this call in a goroutine (see commented out lines),
	// or do it serially (current solution)
...
func (n *Node) HandleMinerBlock(b *block.Block) {
  // 1. update the SeenBlocks 

	// 2. send to chain
	
	// 3. send a newly safe block to the wallet, appending
	// the new block to unsafe blocks
	
	// 4. send to network to broadcast
