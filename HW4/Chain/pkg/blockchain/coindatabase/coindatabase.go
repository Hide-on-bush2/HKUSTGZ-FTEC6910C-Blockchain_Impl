...
func (coinDB *CoinDatabase) validateTransaction(transaction *block.Transaction) error {
  //1. iterate the inputs of the transaction
	for _, txi := range transaction.Inputs {
    //2. generate the coin locator
		
    //3. check whether the coin exits in the main cache, if yes, 
    //check whether the coin is spent
		
    //4. check whether the coin exits in the disk
      ...
      //5. deserilize the bytes to get the encoded transaction
			
      //6. decode the encoded transaction to get the decoded transaction
			
      //7. check whether the coin exits in this transaction
	...
	for i := 0; i < len(blocks); i++ {
		...
		for _, tx := range blocks[i].Transactions {
			...
			for j := 0; j < len(tx.Outputs); j++ {
				// 3. create the coin locator
				
				// 4. Remove the coin from the main cache (if it exists there) and update size
				
			}
			// 5. delete the coin record from the db (erases all the coins in the process)
			
		}
		// 6. deal with UndoBlocks
		for j := 0; j < len(undoBlocks[i].TransactionInputHashes); j++ {
			txHash := undoBlocks[i].TransactionInputHashes[j]
			// 7. retrieve coin record from db
			cr := coinDB.getCoinRecordFromDB(txHash)
			//
			if cr != nil {
				// 8. Add coins to record. This is the reestablishing part.
				
				// 9 .Mark the coin in the main cache as unspent
				
			} else {
				// 9. if there was no coin record to get from the db, we
				// need to make a new one with all the coins from
				// the undoBlock
				
			}
			// 10. put the updated record back in the db.
...
func (coinDB *CoinDatabase) updateSpentCoins(transactions []*block.Transaction) {
	// 1. loop through all the transactions from the block,
	// marking the coins used to create the inputs as spent.
	for _, tx := range transactions {
		for _, txi := range tx.Inputs {
			// 2. get the coin locator for the input
			// 3. mark coins in the main cache as spent,
